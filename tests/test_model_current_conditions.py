...
def test_air_quality_forecast_snapshot(
    snapshot: SnapshotAssertion, air_quality_forecast_data: dict
) -> None:
    """Testing a snapshot of air quality data."""
    data = AirQualityForecastData.from_dict(air_quality_forecast_data)
    ...
    all_options = [idx.category_options for idx in data.hourly_forecasts[0].indexes]
    assert all_options == snapshot(name="indexes_category_options")

    all_pollutant_options = [
        idx.pollutant_options for idx in data.hourly_forecasts[0].indexes
    ]
    assert all_pollutant_options == snapshot(name="indexes_pollutant_options")
